			...
			if (selected.length !== 0) {
				...
				for (const node of selected) {
					...
					if (node.type === 'TEXT') {
						...
						await Promise.all(
							node.getRangeAllFontNames(0, node.characters.length).map(font => figma.loadFontAsync(font))
						)
						...
						switch (true) {
							case msg.fitToGridChecked:
								...
								// Ignore line width if option is checked
								if (msg.ignoreLineWidthChecked) {
									goldenLineHeight = Math.round((fontSize * 1.618) / 8) * 8
								}
								break

							case msg.ignoreLineWidthChecked:
								goldenLineHeight = roundToDecimal(fontSize * 1.618, 1)
								break
